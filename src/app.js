// --- YOUR FIREBASE CONFIG ---
        ...
        function exportToExcel() {
            ...
            const dataToExport = actions.map((action, index) => {
                const person = persons.find(p => p.id === action.assignedPerson);

                // Notlar - her not için yeni satır, özellikle postpone'lar için
                const notesText = action.notes && action.notes.length > 0
                    ? action.notes.map(note => {
                        if (note.type && note.type.toLowerCase() === 'postpone') {
                            // Postpone notları için özel formatla yeni satır
                            return `[ERTELEME] ${note.text}`;
                        } else {
                            return `[${note.type ? note.type.toUpperCase() : 'NOT'}] ${note.text}`;
                        }
                    }).join('\n')
                    : '';
                ...
                return {
                    ...
                    'Hedef Tarih': action.date ? formatDate(action.date) : '',
            ...
            // Column genişlikleri ayarla - sadeleştirilmiş
            const columnWidths = [
                { wch: 8 },  // Sıra
                { wch: 35 }, // Başlık
                { wch: 18 }, // Sorumlu Takım
                { wch: 18 }, // Atanan Kişi
                { wch: 45 }, // Açıklama
                { wch: 15 }, // Durum
                { wch: 15 }, // Hedef Tarih
            ...
            XLSX.utils.book_append_sheet(workbook, worksheet, "Aksiyon Detay Listesi");

            const filename = `AksiyonListesi_${new Date().toLocaleDateString('tr-TR').replace(/\./g, '-')}.xlsx`;
        ...
        function exportToWord() {
            ...
            const timeStr = currentDate.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' });

            // Sadeleştirilmiş Word export formatı
            const dataToExport = actions.map((action, index) => {
                const person = persons.find(p => p.id === action.assignedPerson);

                // Notlar - her not için yeni satır, özellikle postpone'lar için (Word için)
                const notesText = action.notes && action.notes.length > 0
                    ? action.notes.map(note => {
                        if (note.type && note.type.toLowerCase() === 'postpone') {
                            // Postpone notları için özel formatla yeni satır
                ...
                return {
                    ...
                    assignedPerson: person ? person.name : 'Atanmamış',
                    status: getStatusText(action.status) || '',
                    priority: action.priority ? 'Evet' : 'Hayır',
            ...
            const anadoluActions = dataToExport.filter(a => a.responsible === "Anadolu Bakır");
            const aifteamActions = dataToExport.filter(a => a.responsible === "AIFTEAM");
            const ortakActions = dataToExport.filter(a => a.responsible && a.responsible.includes("Ortak"));
            ...
            let htmlContent = `
                ...
                <head>
                    <meta charset='utf-8'>
                    <title>Aksiyon Takip Raporu - Detaylı</title>
                    <style>
                        body {
                        ...
                        .status.completed, .status.tamamlandi {
                            background-color: #27ae60;
                            color: white;
                        }
                        .status.in-progress, .status.devam-ediyor {
                            background-color: #3498db;
                            color: white;
                        }
                        .status.waiting, .status.bekliyor {
                            background-color: #f39c12;
                            color: white;
                        }
                        .notes-section, .attachments-section, .postpone-section {
                            background-color: #ecf0f1;
                            padding: 6px;
                            margin: 4px 0;
                            border-radius: 3px;
                ...
                <body>
                    <h1>📋 Aksiyon Takip Raporu - Detaylı</h1>
                    <p><strong>Rapor Tarihi:</strong> ${reportDate} | <strong>Toplam Aksiyon:</strong> ${actions.length}</p>
                    ...
                    <table class="stats-table">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>${anadoluActions.filter(a => a.status === 'Tamamlandı').length}</td>
                                <td>${anadoluActions.filter(a => a.status === 'Devam Ediyor').length}</td>
                                <td>${anadoluActions.filter(a => a.status === 'Bekliyor').length}</td>
                            </tr>
                            <tr>
                                <td>AIFTEAM</td>
                                <td>${aifteamActions.length}</td>
                                <td>${aifteamActions.filter(a => a.status === 'Tamamlandı').length}</td>
                                <td>${aifteamActions.filter(a => a.status === 'Devam Ediyor').length}</td>
                                <td>${aifteamActions.filter(a => a.status === 'Bekliyor').length}</td>
                            </tr>
                            <tr>
                                <td>Ortak</td>
                                <td>${ortakActions.length}</td>
                                <td>${ortakActions.filter(a => a.status === 'Tamamlandı').length}</td>
                                <td>${ortakActions.filter(a => a.status === 'Devam Ediyor').length}</td>
                                <td>${ortakActions.filter(a => a.status === 'Bekliyor').length}</td>
                            </tr>
                            <tr style="font-weight: bold; background-color: #ecf0f1;">
                                <td>TOPLAM</td>
                                <td>${dataToExport.length}</td>
                                <td>${dataToExport.filter(a => a.status === 'Tamamlandı').length}</td>
                                <td>${dataToExport.filter(a => a.status === 'Devam Ediyor').length}</td>
                                <td>${dataToExport.filter(a => a.status === 'Bekliyor').length}</td>
                            </tr>
                        </tbody>
                    </table>
            `;

            // Helper function to add action details - sadeleştirilmiş
            function addActionDetails(action) {
                return `
                    <div class="action-item team-${action.responsible.toLowerCase().replace(' ', '').replace('ı', 'i')}">
                        <div class="action-title">${action.sira}. ${action.title}</div>

                        <div class="action-field">
                            <span class="field-label">Durum:</span>
                            <span class="field-value">
                                <span class="status ${action.status.toLowerCase().replace(' ', '-').replace('ğ', 'g').replace('ı', 'i').replace('ö', 'o').replace('ü', 'u').replace('ç', 'c').replace('ş', 's')}">${action.status}</span>
                            </span>
                        </div>

                        <div class="action-field">
                            <span class="field-label">Sorumlu:</span>
                            <span class="field-value">${action.responsible}</span>
                        </div>

                        <div class="action-field">
                            <span class="field-label">Atanan Kişi:</span>
                            <span class="field-value">${action.assignedPerson}</span>
                        </div>

                        <div class="action-field">
                            <span class="field-label">Hedef Tarih:</span>
                            <span class="field-value">${action.date || "Belirtilmemiş"}</span>
                        </div>

                        <div class="action-field">
                            <span class="field-label">Öncelikli:</span>
                            <span class="field-value">${action.priority}</span>
                        </div>

                        <div class="action-field">
                            <span class="field-label">Kategori:</span>
                            <span class="field-value">${action.category || "Genel"}</span>
                        </div>

                        <div class="action-field">
                            <span class="field-label">Açıklama:</span>
                            <div class="field-value">${action.description || "Açıklama girilmemiş"}</div>
                        </div>
            ...
            if (anadoluActions.length > 0) {
                ...
                anadoluActions.forEach((action, index) => {
                    action.sira = index + 1; // Takım içi sıra numarası
                    htmlContent += addActionDetails(action);
                });
            }
            ...
            // Simple Word export using blob with updated filename
            const blob = new Blob(['\ufeff', htmlContent], { type: 'application/msword' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            const fileName = `AksiyonTakipRaporu_${dateStr}_${timeStr.replace(':', '')}.doc`;
            link.href = url;
            link.download = fileName;
            document.body.appendChild(link);
            link.click();
        ...
        function openPasswordModal() {
            ...
            const modernUserDropdown = document.getElementById('modernUserDropdown');
            if (modernUserDropdown) {
                modernUserDropdown.style.display = 'none';
            }
        }
        ...
        // Test için eksik fonksiyonları ekle
        ...
        function searchActions(searchTerm = '') {
            ...
            return filteredActions.filter(action =>
                ...
                (action.assignedPerson && action.assignedPerson.toLowerCase().includes(searchTerm))
            );
        }
